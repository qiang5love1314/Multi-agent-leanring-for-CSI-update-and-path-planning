...
from Lab.Lab_GAN import *
from Lab.LabRewardCalculator import *
import time
...
def accuracyPre(predictions, labels):
    accuracy = np.mean(np.sqrt(np.sum((predictions-labels) ** 2, 1))) * 50 / 100
...
# optimal_path = np.load('/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Lab/final_paths.npy', allow_pickle=True)
# optimal_path = np.load('/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Experiments/PSRO_Path_Analysis/Lab_iter50_final_paths.npy', allow_pickle=True)
# optimal_path = np.load('/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Experiments/AgentNum_Analysis/Lab/Optimal50iter_(1,1)(12,1)(21,1)_final_path.npy',allow_pickle=True)
optimal_path = np.load('/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Experiments/AgentNum_Analysis/2Agents_Lab_final_path.npy',allow_pickle=True)

CPPU_label = np.array(optimal_path.tolist(), dtype=object)
CPPU_label_flat = np.vstack(CPPU_label)
raw_csi_label = np.unique(np.concatenate((gauss_label, CPPU_label_flat), axis=0), axis=0)  # 117 labels, i.e., real sampling points
indices = [np.where(np.all(label == point, axis=1))[0][0] for point in raw_csi_label]
raw_csi = original_data[indices]
# print(len(raw_csi_label))

# random sampling strategy for the reviewer
# np.random.seed(3)
# random_indices = np.random.choice(len(label), size=int(0.1 * len(label)), replace=False)  # 选择10%/30%/50%的路径点
...
# saveTestErrorMat(prediction, testLabel, '/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Experiments/AgentNum_Analysis/Lab/50iter-Lab-(1,1)(12,1)(21,1)-Error')
# saveTestErrorMat(prediction, testLabel, '/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Experiments/AgentNum_Analysis/2Agents_10iter-Lab--Error')
# saveTestErrorMat(prediction, testLabel, '/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/Experiments/RandomSampling_Analysis/Lab_percent_10-Error')
