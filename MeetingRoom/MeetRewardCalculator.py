...
from MeetingRoom.MeetMulAgtPath import find_PotentialPath

class RewardCalculator:
    np.random.seed(123)
    def __init__(self, path="/Users/zhuxiaoqiang/Downloads/55SwapData/"):
        self.path = path

    def get_x_label(self):
        x_label = [str(i + 1) for i in range(16)]  # 横坐标
        return x_label

    def get_y_label(self):
        y_label = [f'{0}{j + 1}' if j < 9 else str(j + 1) for j in range(11)]  # 纵坐标
        return y_label
    ...
    def raw_csi(self):
        ...
        count = 0
        original_csi = np.zeros((176, 135000), dtype=np.float)
    ...
    def calculateRewards(self):
        ...
        for i in range(16):
            for j in range(11):
                filePath = f"/Users/zhuxiaoqiang/Downloads/55SwapData/coordinate" + xLabel[i] + yLabel[j] + ".mat"
    ...
    def diagReward_selected(self):
        diagReward = np.load('/Users/zhuxiaoqiang/Desktop/IEEE Trans/BJTU-third multi agent/main/MeetingRoom/Meet_rewards.npy', allow_pickle=True)  # 已计算直接加载
    ...
    def get_coordinatesList(self):
        coordinateList = np.array(find_PotentialPath(), dtype=object) # 各机器人可移动范围，array（list1_length=70，list2_length=35，list3_length=71
